...
#include "offsets.h"
#include "xstring"
#include "C:\Users\a\Downloads\PasterX\x64\Release\paste\Fn\lazy.h"
...

{
  ...
  Log3(skCrypt(" Press [ F2 ] To Load The Cheat\n"));
  std::cout << skCrypt("\n");
...
void DrawESP() {
	...
	if (square_fov) {
		ImGui::GetOverlayDrawList()->AddRect(ImVec2(ScreenCenterX - AimFOV, ScreenCenterY - AimFOV), ImVec2(ScreenCenterX + AimFOV, ScreenCenterY + AimFOV), IM_COL32(89, 3, 247, 255), 1.5f);
		bool fovcircle = false;
		bool  fovcirclefilled = false;
	}
	if (fovcircle) {
		ImGui::GetOverlayDrawList()->AddCircle(ImVec2(ScreenCenterX, ScreenCenterY), float(AimFOV), ImColor(89, 3, 247, 255), 100.0f, 1.5f);
		bool square_fov = false;
		bool fovcircle = false;
		bool  fovcirclefilled = false;
	}
   ...
   // if (read<float>(CurrentActor + OFFSETS::Revivefromdbnotime) != 10) continue;
	...
	if (distance < VisDist)
	{
		if (Esp)
		{
			...
			{
					DrawLine(Width / 2, Height, bottom.x, bottom.y, &Col.purple, 1.5);
...
void render() {
	...
	if (ShowMenu)
	{
		...
		XorS(menu, "PasterX Free | Discord.gg/Pasterx  |  " __DATE__ );
		XorS(box_esp, "Box");
		...
		if (process_find(_xor_("FortniteClient-Win64-Shipping_EAC.exe"))) {
			ImGui::Text("You Have Loaded With EAC");
		}
		if (process_find(_xor_("FortniteClient-Win64-Shipping_BE.exe"))) {
			ImGui::Text("You Have Loaded With BE");
		}
		ImGui::Spacing();
